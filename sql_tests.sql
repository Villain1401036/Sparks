...
select createddate as date , count(*) from  (select Date(createdAt) as createddate from edw.messages) a group by createddate 


-- Are there any users that did not receive any message?
select id from users where not exists (select distinct receiverId from edw.messages )


-- How many active subscriptions do we have today?
select count(*) from edw.subscriptions where status = 'Active'
...
select user_id from edw.subscriptions s inner join edw.messages me on me.senderId = s.user_id  where status = 'Active' and me.createdAt not between s.startDate and EndDate  
...
select createdMonth , sum(amount)/count(*) as avgprice from (select id, amount , to_char(createdDate, 'YYYY-MM') as createdMonth from edw.subscription where status <> 'Rejected' ) group by createdMonth order by createdMonth
